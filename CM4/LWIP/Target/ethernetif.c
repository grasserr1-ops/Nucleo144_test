...
/* USER CODE BEGIN 0 */
/*
 * CubeMX/HAL mismatch workaround (re-apply after regenerate if build fails):
 * - Remove undeclared TxConfig init block from low_level_init()
 * - Use ETH_TxPacketConfigTypeDef (not ETH_TxPacketConfig) in low_level_output()
 */
/* USER CODE END 0 */
...
#define INTERFACE_THREAD_STACK_SIZE ( 350 * 4 )
